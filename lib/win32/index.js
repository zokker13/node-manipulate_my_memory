const manipulateMemory = require('bindings')('Win32ManipulateMyMemory');
...
function readProcessMemorySync(handle, address, size) {
  return manipulateMemory.readProcessMemorySync(handle, address, size);
}

function writeProcessMemory(handle, address, size, newBuffer, cb) {
  manipulateMemory.writeProcessMemory(handle, address, size, newBuffer, cb);
}

function writeProcessMemorySync(handle, address, size, newBuffer) {
  return manipulateMemory.writeProcessMemorySync(handle, address, size, newBuffer);
}
...
function getWindowThreadProcessId(hwnd, cb) {
  manipulateMemory.getWindowThreadProcessId(hwnd, cb);
}

function getWindowThreadProcessIdSync(hwnd) {
  return manipulateMemory.getWindowThreadProcessIdSync(hwnd);
}
...
module.exports = {
  ...
  , writeProcessMemory
  , writeProcessMemorySync
  , closeHandle
  , closeHandleSync
  , findWindow
  , findWindowSync
  , getWindowThreadProcessId
  , getWindowThreadProcessIdSync
};
