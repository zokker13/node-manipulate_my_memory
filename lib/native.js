...
const steps = [
  ...
  (handle, cb) => {
    readIntWrap(handle, cb);
  },
  (handle, cb) => {
    readUIntWrap(handle, cb);
  },
  (handle, cb) => {
    manipulateMyMemoryAddon.closeHandle(handle, cb);
  }
];
...
function rpm(handle, cb) {
  ...
  console.log('rpm yo');
  manipulateMyMemoryAddon.readProcessMemory(handle, 0x01818650  , 4, (err, success, buf) => {
    if (err !== null) {
      console.log('Shit:', err);
      cb(err);
...
function readIntWrap(handle, cb) {
  readInt(handle, 0x01818650, 4, (err, number) => {
    console.log(number);
    cb(err, handle);
  });
}
...
function readUIntWrap(handle, cb) {
  readUInt(handle, 0x01818650, 4, (err, number) => {
    console.log(number);
    cb(err, handle);
  });
}
